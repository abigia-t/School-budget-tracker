const RoleRoute = {
  "System Admin": "/system-admin-page/system-admin-dashboard",
  "General Manager": "/general-manager-page/general-manager-dashboard",
  "School Director": "/school-director-page/school-director-dashboard",
  Auditor: "/auditor-page/auditor-dashboard",
  "Resource and Finance":
    "/resource-and-finance-head-page/resource-and-finance-head-dashboard",
  "Human Resource": "/human-resource-page/human-resource-dashboard",
  Student: "/parent/dashboard", // Student route is included
};

export default RoleRoute;
