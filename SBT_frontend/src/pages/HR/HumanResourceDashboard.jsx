...
const HumanResourceDashboard = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.data) {
          ...
          if (validMessages.length > 0) {
            ...
            setMessages([]);
            toast.info("No messages found in your inbox");
          }
        } else {
          setMessages([]);
          toast.info("Your message inbox is empty");
        }
      } catch (error) {
        console.error("Error fetching messages:", error);
        setFetchError(error.response?.data?.message || "Failed to load messages. Please try again.");
        setMessages([]);
      } finally {
    ...
    if (user?.email) {
      ...
      setFetchError("Please login to view messages");
      setIsLoadingMessages(false);
    }
  }, [user]);
